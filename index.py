...
def index():
    ...
    return render_template('car_user.html', username=session['username'], title='Каталог товаров', cars=cars)
...
def add_car():
    ...
    return render_template("add_car.html", title='Добавление продукта', form=form)
...
def search_price():
    ...
    if form.validate_on_submit():
        ...
        return render_template('car_user.html', username=session['username'], title='Просмотр каталога', cars=cars)
...
def search_dealer():
    ...
    if form.validate_on_submit():
        ...
        # редирект на главную страницу
        return render_template('car_user.html', username=session['username'], title='Просмотр каталога', cars=cars)
    return render_template("search_dealer.html", title='Подбор по магазину', form=form)
